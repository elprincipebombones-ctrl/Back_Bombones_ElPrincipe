...
exports.crearSeguimiento = async (req, res, next) => {
  try {
    const accion = await AccionCorrectiva.findByPk(req.params.id, {
      include: [{
        ...
        include: [{
          model: ReglaCalidad,
          as: 'regla',
          include: [{ model: CondicionRegla, as: 'condiciones', where: { estado: true }, required: false }],
        }],
      }],
    });
    ...
    if (tipoRegistro === 'NUEVA_MEDICION') {
      if (valorNumero === null) return fail(res, 'Debes ingresar el valor de la nueva medición', 422);
