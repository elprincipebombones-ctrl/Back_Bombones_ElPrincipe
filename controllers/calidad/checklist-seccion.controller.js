...
const {
  validarSeccion,
  validarChecklist,
} = require('../../services/calidad/inmutabilidad-version.service');
...
const sincronizarElementos = async (checklistId, ids, transaction) => {
  ...
  await ElementoChecklist.bulkCreate(
    ids.map((elementoId, indice) => ({
...
const validarReferencias = async (body, transaction) => {
  ...
  if (
    body.seccionFormatoId &&
    !(await SeccionFormato.findByPk(body.seccionFormatoId, { transaction }))
  ) {
    return 'Sección no encontrada';
  }
  if (body.criterioInspeccionId) {
    const criterio = await CriterioInspeccion.findOne({
      where: { id: body.criterioInspeccionId, estado: true },
      transaction,
    });
...
exports.listar = async (_req, res, next) => {
  try {
    return ok(res, await ChecklistSeccion.findAll({ include, order: [['orden', 'ASC']] }));
  } catch (error) {
    return next(error);
  }
};
