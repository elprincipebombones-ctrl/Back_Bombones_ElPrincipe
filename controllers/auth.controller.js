const { Usuario, Rol, Permiso, Menu, Cargo } = require('../models');
...
exports.login = async (req, res, next) => {
  try {
    ...
    const usuario = await Usuario.scope('withPassword').findOne({
      where: { usuario: identificador.trim().toLowerCase() },
      include: [
...
exports.refresh = async (req, res, next) => {
  try {
    ...
    const payload = { id: usuario.id, usuario: usuario.usuario, correo: usuario.correo };
    return ok(res, {
