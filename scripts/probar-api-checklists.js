...
const request = async (ruta, options = {}) => {
  ...
  assert(
    response.ok,
    `${options.method || 'GET'} ${ruta}: ${response.status} ${JSON.stringify(body)}`,
  );
  return body.data;
};
...
(async () => {
  ...
  const elementos = await request(
    `/calidad/elementos-inspeccion?categoria_id=${equipos.id}&estado=true`,
  );
  ...
  const validarFormato = async (codigo, esperados) => {
    ...
    const cantidades = completa.secciones
      .flatMap((seccion) => seccion.checklists || [])
      .map((checklist) => checklist.elementos.length)
      .sort((a, b) => a - b);
