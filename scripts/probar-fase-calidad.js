...
const contarContenido = async (versionId, transaction) => {
  const secciones = await db.SeccionFormato.findAll({ where: { versionFormatoId: versionId }, transaction });
  const seccionIds = secciones.map((item) => item.id);
  const campos = await db.CampoFormato.findAll({ where: { seccionFormatoId: seccionIds }, transaction });
  const checklists = await db.ChecklistSeccion.findAll({ where: { seccionFormatoId: seccionIds }, transaction });
  const elementos = await db.ElementoChecklist.count({
    where: { checklistSeccionId: checklists.map((item) => item.id) },
    transaction,
  });
  return { secciones: secciones.length, campos: campos.length, checklists: checklists.length, elementos };
};
...
const main = async () => {
  ...
  try {
    ...
    assert(tipoInspeccion && tipoCampo && criterio && elemento && usuario, 'Faltan catálogos base de Calidad');
    ...
    await db.DiaProgramacion.bulkCreate([1, 2, 3, 4, 5, 6].map((diaSemana) => ({
    ...
    assert.deepEqual(await contarContenido(v2.id, transaction), await contarContenido(v1.id, transaction));
    ...
    assert.equal(await db.VersionFormato.count({
    ...
    assert.equal(await db.RespuestaElementoChecklist.count({ where: { inspeccionId: actual.id }, transaction }), 0);
    ...
    const elementoCopiado = await db.ElementoChecklist.findOne({
      include: [{
        ...
        as: 'checklist',
        where: { seccionFormatoId: { [Op.in]: (await db.SeccionFormato.findAll({ where: { versionFormatoId: v2.id }, transaction })).map(s => s.id) } },
      }],
      transaction,
    });
    ...
    const evidenciaTipo = await db.TipoAccion.findOne({ where: { codigo: 'EXIGIR_EVIDENCIA' }, transaction });
    const medicionTipo = await db.TipoAccion.findOne({ where: { codigo: 'VOLVER_A_MEDIR' }, transaction });
