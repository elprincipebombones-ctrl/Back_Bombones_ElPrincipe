...
const ejecutar = async () => {
  ...
  try {
    ...
    const formato = await db.FormatoCalidad.findOne({
      where: { codigo: 'TEMP-CAVA' },
      transaction,
    });
    ...
    const seccion = await db.SeccionFormato.findOne({
      where: { versionFormatoId: version.id },
      transaction,
    });
    assert(usuario && formato && version && seccion, 'Faltan datos base para la prueba');

    const asegurarCampo = async (codigoParametro) => {
      const parametro = await db.ParametroCalidad.findOne({
        where: { codigo: codigoParametro },
        transaction,
      });
      assert(parametro, `No existe el parámetro ${codigoParametro}`);
      let campo = await db.CampoFormato.findOne({
        where: { parametroCalidadId: parametro.id },
        transaction,
      });
      if (!campo)
        campo = await db.CampoFormato.create(
          {
            ...
            codigo: `QA_${codigoParametro}`.slice(0, 30),
    ...
    const probar = async (codigoParametro, valor, esperado, accionesMinimas = []) => {
      ...
      const inspeccion = await db.Inspeccion.create(
        {
          ...
          estado: 'BORRADOR',
          fechaInspeccion: '2026-08-16',
          iniciadaPor: usuario.id,
          fechaInicio: new Date(),
          observaciones: 'Prueba transaccional',
        },
        { transaction },
      );
      ...
      const desviaciones = await db.Desviacion.findAll({
      ...
      assert.equal(
        desviaciones.length,
        esperado,
        `${codigoParametro}=${valor}: desviaciones inesperadas`,
      );
      if (esperado) {
        const acciones = await db.AccionCorrectiva.findAll({
          where: { desviacionId: desviaciones[0].id },
          include: [{ model: db.TipoAccion, as: 'tipoAccion' }],
          transaction,
        });
    ...
    const resultados = [];
    resultados.push(
      await probar('CLORO_AGUA_POTABLE', 0.2, 1, ['GENERAR_ALERTA', 'VOLVER_A_MEDIR']),
    );
