...
const ejecutar = async () => {
  ...
  assert(
    reglas.every((r) => r.parametroCalidadId),
    'Hay reglas sin parámetro',
  );
  assert(
    reglas.every((r) => r.parametro && r.condiciones && r.acciones),
    'El detalle de reglas está incompleto',
  );
  ...
  const completa = await solicitar(`/calidad/versiones-formato/${publicada.id}/completa`, {
    headers,
  });
  const campoTemperatura = completa.secciones
    .flatMap((s) => s.campos)
    .find((c) => c.parametro?.codigo === 'TEMPERATURA');
