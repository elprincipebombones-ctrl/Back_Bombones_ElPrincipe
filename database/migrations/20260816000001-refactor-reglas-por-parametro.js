...
module.exports = {
  async up(queryInterface, Sequelize) {
    await queryInterface.sequelize.transaction(async (transaction) => {
      ...
      await queryInterface.addColumn('parametros_calidad', 'tipo_campo_id', {
        type: Sequelize.UUID,
        allowNull: true,
      ...
      await queryInterface.addColumn('parametros_calidad', 'valor_minimo', { type: Sequelize.DECIMAL, allowNull: true }, options);
      await queryInterface.addColumn('parametros_calidad', 'valor_maximo', { type: Sequelize.DECIMAL, allowNull: true }, options);
      await queryInterface.addColumn('parametros_calidad', 'precision_decimal', { type: Sequelize.INTEGER, allowNull: true }, options);
      await queryInterface.addColumn('parametros_calidad', 'es_obligatorio_default', { type: Sequelize.BOOLEAN, allowNull: false, defaultValue: false }, options);
      await queryInterface.addColumn('parametros_calidad', 'permite_observacion_default', { type: Sequelize.BOOLEAN, allowNull: false, defaultValue: true }, options);
      await queryInterface.addColumn('parametros_calidad', 'requiere_evidencia_default', { type: Sequelize.BOOLEAN, allowNull: false, defaultValue: false }, options);
      await queryInterface.addColumn('parametros_calidad', 'bloquear_al_guardar_default', { type: Sequelize.BOOLEAN, allowNull: false, defaultValue: false }, options);

      await queryInterface.addColumn('reglas_calidad', 'parametro_calidad_id', {
        type: Sequelize.UUID,
        allowNull: true,
      ...
      await queryInterface.addColumn('reglas_calidad', 'resultado', { type: Sequelize.STRING(20), allowNull: true }, options);
      await queryInterface.sequelize.query(
        'ALTER TABLE reglas_calidad ALTER COLUMN campo_formato_id DROP NOT NULL', options,
      );
      ...
      await queryInterface.removeConstraint('reglas_calidad', 'uq_reglas_calidad_campo_codigo', options);
      await queryInterface.addIndex('parametros_calidad', ['tipo_campo_id'], { name: 'ix_parametros_calidad_tipo_campo', transaction });
      await queryInterface.addIndex('reglas_calidad', ['parametro_calidad_id'], { name: 'ix_reglas_calidad_parametro', transaction });
      ...
      const [[volver]] = await queryInterface.sequelize.query(
        `SELECT id FROM tipos_accion WHERE codigo = 'VOLVER_A_MEDIR'`, options,
      );
      if (!volver) {
        await queryInterface.bulkInsert('tipos_accion', [{
          id: uuidv4(), codigo: 'VOLVER_A_MEDIR', nombre: 'Volver a medir',
          descripcion: 'Solicita registrar una nueva medición de verificación', estado: true, ...comun,
        }], options);
      }
      ...
      const [[cloroActual]] = await queryInterface.sequelize.query(
        `SELECT id FROM parametros_calidad WHERE codigo IN ('CLORO_AGUA_POTABLE', 'CLORO')
         ORDER BY CASE WHEN codigo = 'CLORO_AGUA_POTABLE' THEN 0 ELSE 1 END LIMIT 1`, options,
      );
      ...
      const crearEscenario = async ({ parametroId, codigo, nombre, operador, valor1, valor2,
        resultado, mensaje, acciones, descripcionAccion }) => {
        ...
        if (existente) {
          ...
          `, { replacements: { nombre, severidad: resultado === 'NO_CUMPLE' ? altaId : null,
            mensaje, resultado, ahora, id: reglaId }, transaction });
        } else {
          await queryInterface.bulkInsert('reglas_calidad', [{
            id: reglaId, parametro_calidad_id: parametroId, campo_formato_id: null,
            codigo, nombre, descripcion: null,
            nivel_severidad_id: resultado === 'NO_CUMPLE' ? altaId : null,
            mensaje_incumplimiento: mensaje, operador_logico: 'AND', resultado, estado: true, ...comun,
          }], options);
        }
        await queryInterface.bulkDelete('condiciones_regla', { regla_calidad_id: reglaId }, options);
        ...
        await queryInterface.bulkInsert('condiciones_regla', [{
          id: uuidv4(), regla_calidad_id: reglaId, tipo_condicion: 'RANGO', operador,
          valor_1: String(valor1), valor_2: valor2 === null ? null : String(valor2),
          valor_json: null, orden: 1, estado: true, ...comun,
        }], options);
        if (acciones.length) {
          await queryInterface.bulkInsert('acciones_regla', acciones.map((codigoAccion, indice) => ({
            id: uuidv4(), regla_calidad_id: reglaId, tipo_accion_id: accionIds[codigoAccion],
            configuracion: descripcionAccion && indice === 0
              ? JSON.stringify({ descripcion: descripcionAccion }) : null,
            orden: indice + 1, estado: true, ...comun,
          })), options);
        }
      };

      const escenarios = [
        { parametroId: cloroActual.id, codigo: 'CLORO_BAJO', nombre: 'Cloro por debajo del rango', operador: 'MENOR_QUE', valor1: 0.3, valor2: null, resultado: 'NO_CUMPLE', mensaje: 'Cloro residual por debajo de 0.3 mg/l.', acciones: ['GENERAR_ALERTA', 'VOLVER_A_MEDIR', 'SOLICITAR_ACCION_CORRECTIVA'] },
        { parametroId: cloroActual.id, codigo: 'CLORO_NORMAL', nombre: 'Cloro dentro del rango', operador: 'ENTRE', valor1: 0.3, valor2: 2, resultado: 'CUMPLE', mensaje: 'Valor dentro del rango permitido.', acciones: [] },
        { parametroId: cloroActual.id, codigo: 'CLORO_ALTO', nombre: 'Cloro por encima del rango', operador: 'MAYOR_QUE', valor1: 2, valor2: null, resultado: 'NO_CUMPLE', mensaje: 'Cloro residual superior a 2.0 mg/l.', acciones: ['GENERAR_ALERTA', 'SOLICITAR_ACCION_CORRECTIVA'], descripcionAccion: 'Ajustar concentración de cloro.' },
        { parametroId: phId, codigo: 'PH_BAJO', nombre: 'pH por debajo del rango', operador: 'MENOR_QUE', valor1: 6.5, valor2: null, resultado: 'NO_CUMPLE', mensaje: 'pH por debajo de 6.5.', acciones: ['GENERAR_ALERTA', 'VOLVER_A_MEDIR', 'SOLICITAR_ACCION_CORRECTIVA'] },
        { parametroId: phId, codigo: 'PH_NORMAL', nombre: 'pH dentro del rango', operador: 'ENTRE', valor1: 6.5, valor2: 9, resultado: 'CUMPLE', mensaje: 'Valor dentro del rango permitido.', acciones: [] },
        { parametroId: phId, codigo: 'PH_ALTO', nombre: 'pH por encima del rango', operador: 'MAYOR_QUE', valor1: 9, valor2: null, resultado: 'NO_CUMPLE', mensaje: 'pH superior a 9.0.', acciones: ['GENERAR_ALERTA', 'VOLVER_A_MEDIR', 'SOLICITAR_ACCION_CORRECTIVA'] },
      ];
  ...
  async down(queryInterface, _Sequelize) {
    await queryInterface.sequelize.transaction(async (transaction) => {
      const options = { transaction };
      await queryInterface.bulkDelete('reglas_calidad', {
      ...
      `, options);
      await queryInterface.removeConstraint('reglas_calidad', 'ck_reglas_calidad_resultado', options);
      await queryInterface.removeIndex('reglas_calidad', 'uq_reglas_calidad_parametro_codigo', options);
      await queryInterface.removeIndex('reglas_calidad', 'ix_reglas_calidad_parametro', options);
      await queryInterface.removeIndex('parametros_calidad', 'ix_parametros_calidad_tipo_campo', options);
      await queryInterface.addConstraint('reglas_calidad', {
        fields: ['campo_formato_id', 'codigo'], type: 'unique',
        name: 'uq_reglas_calidad_campo_codigo', transaction,
      });
      ...
      await queryInterface.sequelize.query(
        'ALTER TABLE reglas_calidad ALTER COLUMN campo_formato_id SET NOT NULL', options,
      );
      for (const columna of ['bloquear_al_guardar_default', 'requiere_evidencia_default',
        'permite_observacion_default', 'es_obligatorio_default', 'precision_decimal',
        'valor_maximo', 'valor_minimo', 'tipo_campo_id']) {
